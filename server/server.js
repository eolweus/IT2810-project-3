...
app.get('/api/tracks', (req, res) => {
    ...
    if (sortBy === undefined || sortOrder === undefined) {
        queries.search(searchString, limit, offset).then((result)=>res.json(result)).catch((error) => {console.log(error);
        res.send(error)});
    }
      ...
      /*
        ...
        queries.searchWithSorting(searchString, limit, offset, sortBy, sortOrder).then((result)=>res.json(result)).catch((error) => {console.log(error)
        res.send(error)});
    }
